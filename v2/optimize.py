import sys, os, logging

from hyperopt import hp, fmin, tpe
from hyperopt import STATUS_OK, STATUS_FAIL

from ghosh import GhoshModel

LEARN = 'a'
KERNEL = 'kernel'
BATCH = 'batch'

class Optimizer:

    def __init__(self, model, output_dir, train_path, validation_path):
        self.model = model
        self.output_dir = output_dir
        self.train_path = train_path
        self.validation_path = validation_path

    def objective(self, args):

        learn = args[LEARN]
        kernel = args[KERNEL]
        batch = args[BATCH]

        print("Running objective: %s, %s, %s" % (str(learn), str(kernel), str(batch)))
        if learn is None or kernel is None or batch is None:
            return {status: STATUS_FAIL}

        out_path = self.get_output_dir(learn, kernel, batch)

        if not os.path.isdir(out_path):
            os.mkdir(out_path)

        model = self.model(self.train_path,
                           self.validation_path,
                           learning_rate=learn,
                           kernel_size=kernel,
                           batch_size=batch,
                           output_dir=out_path,
                           retrain=True)

        f1_score, accuracy = model.run()

        result = {
            status: STATUS_OK,
            loss: 1 - (sum(f1_score) / float(len(f1_score))),
            attachments: {
                model: model.get_model_path(),
                f1_score: f1_score,
                accuracy: accuracy
            }
        }

    def get_output_dir(self, a, b, c):
        return os.path.join(self.output_dir, "optimize_%s_%s_%s" % (a,b,c))

def main():

    fmt = "%(levelname) -10s %(asctime)s %(module)s:%(lineno)s %(funcName)s %(message)s"
    logging.basicConfig(filename='optimizer.log', level=logging.DEBUG, filemode='w', format=fmt)
    log = logging.getLogger('OPTIMIZER')

    try:
        optimizer = Optimizer(GhoshModel, sys.argv[3], sys.argv[1], sys.argv[2])
    except Exception as e:
        log.error(e)

    print('Defining space')
    space = {
        LEARN: hp.uniform(LEARN, 0.0000001, 0.01),
        KERNEL: hp.quniform(KERNEL, 8, 3, 1),
        BATCH: hp.quniform(BATCH, 128, 8, 1)
    }

    try:
        log.info("Beginning optimization")
        print("Beginning optimization")
        best = fmin(optimizer.objective,
             space=space,
             algo=tpe.suggest,
             max_evals=100)

        log.info(str(best))
    except Exception as e:
        log.error(e)

if __name__ == '__main__':
    main()
